...
fn assign_chain() {
    ...
    exec_str(&scope, r"foo = bar = 1;").unwrap();
...
#[test]
fn if_non_bool_cond() {
    let result = exec_str(&Rc::new(Scope::root()), r"if 1 then return");
    assert!(matches!(
        result,
        Err(ExecError {
...
fn assign_non_literal() {
    let result = exec_str(&Rc::new(Scope::root()), r"+1 = +2");
    assert!(matches!(
        result,
        Err(ExecError {
...
#[test]
fn assign_undef() {
    let result = exec_str(&Rc::new(Scope::root()), r"foo = 1");
    assert!(matches!(
        result,
        Err(ExecError {
            pos: Pos { line: 0, col: 0 },
...
#[test]
fn obj_pat_non_obj() {
    let result = exec_str(&Rc::new(Scope::root()), r"let {x, y} = 1;");
    assert!(matches!(
        result,
        Err(ExecError {
...
#[test]
fn var_undef() {
    let val = eval_str(&Rc::new(Scope::root()), r"foo").unwrap();
...
fn prop_access() {
    ...
    exec_str(&scope, r#"obj = { name: "john", age: 25 };"#).unwrap();
...
fn prop_access_nested() {
    let scope = Rc::new(Scope::root());
    scope.declare("obj", Val::default());
    exec_str(&scope, r#"obj = { user: { name: "Alice" } };"#).unwrap();
